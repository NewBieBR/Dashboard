class NewsByGenre extends Widget {
	constructor(params) {
		super('news_by_genre', params);
	}

	update() {
		var self = this;
		var req =
			'https://newsapi.org/v2/top-headlines?country=fr&category=' +
			this.genre +
			'&apiKey=' +
			NEWS_APIKEY;
		$.ajax({
			...
			success: res => {
				...
				$body.empty();
				$body.append('<div class="news_by_genre"></div>');
				var $cont = $('#widget' + self.id + ' .news_by_genre');
